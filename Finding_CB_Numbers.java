...
As there can be multiple solutions, Gautam asks Deepak to find the maximum number of CB numbers that can be formed from
the given string.
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0 ; i < st.length() ; i++){
            for(int j = i ; j < st.length() ; j++){
                long num = Long.parseLong(st.substring(i , j));
                if(isPrime(num))
                {
                    count++;
